...
class ErrorLogger
{
    ...
    * @var string
    */
    public static $errorFile = '/errors/php-error.log';
    /**
    ...
    */
    public static function newMessage($exception)
    {
        ...
        $logMessage = "Exception information:\n
           ...
           {$trace}\n
           ---------\n\n";
        if (is_file(ROOTDIR.self::$errorFile) === false) {
            file_put_contents(ROOTDIR.self::$errorFile, '');
        }
        if (self::$clear) {
            $f = fopen(ROOTDIR.self::$errorFile, "r+");
            if ($f !== false) {
                ftruncate($f, 0);
                fclose($f);
            }
        ...
        // Append
        file_put_contents(ROOTDIR.self::$errorFile, $logMessage, FILE_APPEND);
